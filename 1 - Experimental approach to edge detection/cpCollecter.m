...
for idxMtype = 1:length(Experiment.params.measure.power.type)
    ...
    %                 if (strcmp(featureMethod,'dCF'))
                    if (ismember(featureMethod,{'dCF','dXCF','dXCF1F2', 'sugeno', 'FGsugeno'}))
        ...
        %                             end
                    ...
                    elseif (strcmp(featureMethod,'ged'))
                        for idxGedFunc = 1:length(Experiment.config.feat.ged.F)
                            cpFileName(idxRes) = {sprintf('%s%s-%s-[%s]-%s-%s-%d-%s-w-%d',Experiment.cpPrefix,rawImageName,smoothingMethod,config,featureMethod,Experiment.config.feat.ged.F{idxGedFunc},Experiment.config.feat.ged.wSize,Experiment.config.feat.ged.kmode,tamW)};
                            idxRes = idxRes+1;
                        end
